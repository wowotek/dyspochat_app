...
<script>
  ...
  export default {
    ...
    methods: {
      delete_room(room_id){
        this.axios({
            url: "http://10.184.0.3:5000/chatroom/"+room_id,
            method: "DELETE",
            headers: {
      ...
      leave_room(room_id){
        this.axios
          .patch("http://10.184.0.3:5000/chatroom", {
              chatroom_id: room_id,
      ...
      message_update(chatroom_id){
        this.axios({
            url: "http://10.184.0.3:5000/chatroom/"+chatroom_id,
            method: "GET",
            headers: {
              'x-api-key': 'wowotek-key',
            }
      ...
      openChatroom(chatroom_id){
        for(var i=0; i<this.chatrooms.length; i++){
          if(this.chatrooms[i].id == chatroom_id){
            this.axios({
                url: "http://10.184.0.3:5000/chatroom/"+this.chatrooms[i].id,
                method: "GET",
                headers: {
                  'x-api-key': 'wowotek-key',
                }
      ...
      handleMenuAction(action, chatroom_data){
        ...
        if(action == "add"){  // Add Chatroom
          this.axios          // Join Chatroom after creating it
            .post("http://10.184.0.3:5000/chatroom", {
            ...
            .then(response => {
                ...
                if (response.data.status == "success"){
                    ...
                    this.axios({
                        url: "http://10.184.0.3:5000/chatroom/"+chatroom_data.id,
                        method: "GET",
                        headers: {
                          'x-api-key': 'wowotek-key',
                        }
